...
<div x-data="{ preview: false, content: @entangle($attributes->wire('model')) }"
     class="wysiwyg-editor border rounded-lg shadow-sm p-4 bg-white"
     id="editor-container-{{ $editorId }}">
    <div class="toolbar flex space-x-2 mb-4 {{ $options ? 'border-b' : '' }} pb-2">
        @foreach ($options as $option)
            <button type="button"
                    class="px-2 py-1 hover:bg-gray-200  text-gray-700 rounded focus:outline-none"
    ...
    <template x-if="!preview">
        <textarea
            ...
            {!! $attributes->merge(['class' => 'bg-white !text-gray-800 border-none !ring-0 !outline-none rounded-md shadow-sm w-full h-64 p-3']) !!}
            id="{{ $editorId }}"
        ></textarea>
    </template>
    <template x-if="preview">
        <div class=" border-none !ring-0 !outline-none rounded-md shadow-sm w-full h-64 p-3" x-html="markdownToHtml(content)"></div>
    </template>
    <button class="text-slate-700" type="button" x-show="!preview" x-on:click="preview = !preview">
        preview
    </button>
    <button class="text-slate-700" type="button" x-show="preview" x-on:click="preview = !preview">
        edit
