        ...
        @foreach ($displayedEmojis as $emoji)
            @if ($allowGuests || auth()->user())
                <button
                    ...
                    class="flex items-center space-x-1 bg-gray-100 p-1 rounded {{ $this->hasUserReacted($emoji) ? '!text-white !bg-blue-500' : '' }}"
                >
            ...
            @else
                <button
                    class="flex items-center space-x-1 bg-gray-100 cursor-default p-1 rounded"
                >
                    <span class="transition-transform transform hover:scale-110">{{ $emoji }}</span>
                    <span class="text-xs">{{ $this->getReactionCount($emoji) }}</span>
                </button>
